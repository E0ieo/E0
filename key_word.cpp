...
int Judge_1 ( string x, string y )//定义一个函数与关键字进行比对，是返回1，不是返回0 
{
...
int Judge_2( char str )//判断是否是字母，是返回1，不是返回0 
{
...
void rank_12(string str)//核心字符比对，并且统计Switch-case的数量 
{
		for(int i = 0; i < 32; i++ )
		{
...
void rank_34( string str )//if_else计数和if_elseif_else嵌套的计数	 
{
	if( Judge_1( str , "else if" ) )//先对比else if因为else if中也包含if 
	{
		top++;
		stack[top] = 2;
	}	
...
int main()
{
	...
	int judge_2( char str);
	void rank_12( string str );//关键字判断以及switch-case计数 
	void rank_34( string str );//if_else计数和if_elseif_else嵌套的计数	 
	while ( getline( myfile ,temp ))//一行行读取 
	{
		istringstream is ( temp );
		string s;
		if( Rank >= 3)
		{
			rank_34( temp );//因为if_else嵌套有空格 
		}
		while( is >> s ) 
		{
			rank_12( s );
